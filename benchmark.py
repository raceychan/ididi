...
def test_bench_mark():
    dg = DependencyGraph()
    dependents = {k: v for k, v in globals().items() if isinstance(v, type)}

    pre = perf_counter()
    for dep in dependents.values():
        dg.node(dep)
    aft = perf_counter()
    ...
    for _ in range(rounds):
        ...
        pre = perf_counter()
        for c in dependents.values():
            _ = dg.resolve(c)
        aft = perf_counter()
        cost = round(aft - pre, 6)
        total_cost += cost

    avg = round(total_cost / rounds, 6)

    print(
        f"\n{avg} seoncds to resolve {len(dependents)} instances, {round(total_cost, 6)} seconds for {rounds} rounds"
    )
